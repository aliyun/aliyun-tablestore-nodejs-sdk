...
TableStore.encoder = {
    ...
    _makeAggregation(agg) {
        ...
        const writer = aggBody.__proto__.constructor.encode(aggBody);
        const properties = {
            name: agg.name,
            type: agg.type,
            body: writer.finish(),
    ...
    _makeGroupBy(groupBy) {
        ...
        const writer = groupByBody.__proto__.constructor.encode(groupByBody);
        const properties = {
            name: groupBy.name,
            type: groupBy.type,
            body: writer.finish(),
    ...
    _makeSorter(aSorter) {
        ...
        if (aSorter.fieldSort) {
            ...
            if (aSorter.fieldSort.missing) {
                fieldSortProperties.missing = TableStore.PlainBufferBuilder.serializeSearchValue(aSorter.fieldSort.missing, "missing")
            }
    ...
    encodeSQLQuery: function (params) {
        const properties = {
            query: params.query,
            version: params.version || TableStore.SQLPayloadVersion.SQL_FLAT_BUFFERS,
        };
