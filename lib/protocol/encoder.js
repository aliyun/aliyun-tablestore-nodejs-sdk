...
TableStore.encoder = {
    ...
    _makeColumnCondition: function (proto, column_condition) {
        ...
        if (column_condition instanceof TableStore.CompositeCondition) {
            ...
            throw new Error("expect CompositeCondition, RelationCondition but not ");
        }
    },
    ...
    _makeSingleColumnCondition(condition) {
        ...
        proto.column_name = condition.column_name;
        proto.column_value = TableStore.PlainBufferBuilder.serializeColumnValue(condition.column_value);
    ...
    _makeCondition: function (proto, condition) {
        ...
        proto.row_existence = condition.row_existence_expectation;
        if (proto.row_existence === undefined || proto.row_existence === null) {
            throw new Error("row_existence_expectation should be one of TableStore.RowExistenceExpectation");
        }

        if (condition.column_condition != null) {
            var pbFilter = new tsFilterProtos.Filter();
            TableStore.encoder._makeColumnCondition(pbFilter, condition.column_condition);
